...
function drawConclusionListTable(data, filter) {
    ...
    for (var i = 0; i < data.length; i++) {     
        ...
        if(compareData.includes(filter)){
...
function drawCurrentCaseViewerTable(data, filter) {
    filter = filter.toLowerCase();
    $("#rdr-ka-current-case-viewer-table").html("<tr><th width='300px'>검사코드 ID</th><th width='80px'>수치 종류</th><th>수치 값</th></tr>");
...
//            var compareData2 = attrConvertedNameList[data[i].attribute].toLowerCase();
            ...
            if(isFiltered && !isMissingValue("rdr-ka-current-case-viewer-missing-checkbox", data[i].value) && isInUsedCondition && morphologyType == data[i].morphologyType){
                var row = $("<tr />");
...
function drawCompareCurrentCaseViewerTable(data, filter) {
    filter = filter.toLowerCase();
    $("#rdr-ka-compare-current-case-viewer-table").html("<tr><th width='110px'>Attribute</th><th>Type</th><th>Value</th></tr>");
    for (var i=0; i<data.length; i++) {
        var compareData = data[i].attribute.toLowerCase();
//        var compareData2 = attrConvertedNameList[data[i].attribute].toLowerCase();
        var isFiltered = false;
...
function drawCompareCaseViewerTable(currentData, validateData, filter) {
    filter = filter.toLowerCase();
    $("#rdr-ka-compare-current-case-viewer-table").html("<tr><th width='110px'>Attribute</th><th>Type</th><th>Value</th></tr>");
    $("#rdr-ka-compare-validate-case-viewer-table").html("<tr><th width='110px'>Attribute</th><th>Type</th><th>Value</th></tr>");
    var validRow = 0;
...
function drawUsedConditionSetTable(data) {
    $("#rdr-ka-used-condition-list-table").html("<tr><th width='300px'>Attribute</th><th>Operator</th><th>Value</th></tr>");
    for (var i=0; i<data.length; i++) {
        var row = $("<tr />");
        row.append($("<td>" + data[i].attribute + "</td>"));
//        row.append($("<td>" + attrConvertedNameList[data[i].attribute] + "</td>"));
...
function drawNewConditionSetTable(data) {
    $(".rdr-ka-new-condition-table").html("<tr><th>Attribute</th><th>Operator</th><th>Value</th></tr>");
    for (var i=0; i<data.length; i++) {
        var row = $("<tr />");
        row.append($("<td>" + data[i].attribute + "</td>"));
//        row.append($("<td>" + attrConvertedNameList[data[i].attribute] + "</td>"));
...
//                headerRow.append($("<th>" + attrName + "<br/>(" + attrConvertedNameList[attrName] + ")</th>"));
                headerRow.append($("<th>" + attrName + "<br/></th>"));
            }
        }
    }
