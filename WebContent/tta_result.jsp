...
<%@ page import="kr.ac.kirc.ekp.bean.TTAQuery"%>
...
if(id==null||id.equals("")){
 ...
 TTAQuery t_query = (TTAQuery)request.getAttribute("TTAquery");
 int relevance_count = 0;
 
 
 ArrayList<String> t_query_list = new ArrayList<String>();
 ArrayList<String> t_query_answer = new ArrayList<String>();
 
 String[] tmp_query_list = t_query.getExpanded().split(",");
 String[] tmp_query_answer = t_query.getAnswer().split(",");
 for (int i=0;i<tmp_query_list.length;i++)
 {
	 t_query_list.add(tmp_query_list[i]);
 }
 for (int i=0;i<tmp_query_answer.length;i++)
 {
	 t_query_answer.add(tmp_query_answer[i]);
 }
 int match =0;
 for(int i=0;i<t_query_list.size();i++)
 {
	 if(t_query_answer.contains(t_query_list.get(i)))
	 {
		 match ++;
	 }
 }
 
 double query_accuracy = (double)match/(double)t_query_answer.size() * 100;
...
<body id="top">
	...
	<div class="wrapper row2">
		...
		<div id="layer_fixed">
			Original Query: <b><%=query_text %></b> <br /> 
			
			Query Expanded: <%=t_query.getExpanded() %> <br />
			Query Answer: <%=t_query.getAnswer() %> <br />
			
			The number of query match: <b><%=match %></b> <br />
			Query Accuracy: <b><%=form.format(query_accuracy)%> % (percent)</b> <br />
		</div>
	</div>
	<!-- ################################################################################################ -->
	<!-- ################################################################################################ -->
	...
	<div class="wrapper row3">
      ...
      -->
        ...
        %>
        		...
        		String relevance_result = dao.isJudgedforTTA(conn, query_id, passage_id, isbn, pageNum);
        		if(relevance_result.equals("Definitely Relevant"))
        		{
        			relevance_count++;
        			//System.out.println("relevance_count: " + relevance_count);
        		}
        %>
					<li>
						<article>
							...
							<nav id="mainav" class="fl_right">
								<ul class="clear">
									...
									<li><b><%=dao.isJudgedforTTA(conn, query_id, passage_id, isbn, pageNum)%></b>
        ...
        %>
        	...
        	<!-- ################################################################################################ -->
				<b>Number of relevance: <%=relevance_count %> out of 10</b> <br />
				<b>Precision: <%=(double)(relevance_count)/10 %></b><br /><br />
				<!-- ################################################################################################ -->
...
%>
				...
				<a href="tta_searcher?pageNum=<%=currentPage-1 %>&query_id=<%=request.getParameter("query_id")%>"><img	src="images/icon/icon_prev.gif" alt="이전 페이지로" /></a>
		...
		for(int i=startPage ; i<=endPage;i++)
		{
			...
			{
%>
				<a href="tta_searcher?pageNum=<%= i %>&query_id=<%=request.getParameter("query_id")%>"><b>[<%= i %>]</b></a>
				<%				
			}else{
			
%>
				<a href="tta_searcher?pageNum=<%= i %>&query_id=<%=request.getParameter("query_id")%>">[<%= i %>]</a>
				<%
 ...
 %>
				...
				<a href="tta_searcher?pageNum=<%=currentPage+1 %>&query_id=<%=request.getParameter("query_id")%>"><img src="images/icon/icon_next.gif" alt="다음 페이지로" /></a>
